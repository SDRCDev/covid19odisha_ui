...
export class XhrInterceptorService implements HttpInterceptor {
  ...
  addBaseUrl(req: HttpRequest<any>) {
    let request = !req.url.includes("covid19odisha") ? req.clone() : req.clone({ url: req.url });
    return request.clone();
  }
  ...
  handle401Error(req: HttpRequest<any>, next: HttpHandler) {
    ...
    // so during refresh token call, the url will be  /covid19odisha/oauth/token,
    // we log out user if refresh token is also expired.
    if (req.url == "/covid19odisha/oauth/token") {
      this.logoutUser();
    }
